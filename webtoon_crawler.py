...
def get_driver_and_soup(url):
    ...
    # print(f'finish __get_driver_and_soup__ ')
    return  driver,soup
...
def main():
    ...
    filtered_titles = webtoon_titles[webtoon_titles['day'] == current_day]  # 현재 요일의 웹툰만 필터링
    ...
    for idx, row in enumerate(filtered_titles.iterrows()):
        ...
        contents = get_webtoon_title_contents(row['title'], row['webtoon_link'])
        comments = run_multi_process_get_comments(contents[['content_link', 'title', 'content_name']].values.tolist(), current_title_num, total_titles)
        all_comments.extend(comments)
        
    load_to_s3(webtoon_titles,'title')
    
    load_to_s3(webtoon_contents, 'contents')
    load_to_s3(webtoon_comments, 'omments')
    time.sleep(3)
