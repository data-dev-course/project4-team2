...
function BarChartRateVer(props) {
    const [valueType, setValueType] = useState("all_tag")
    ...
    useEffect(()=> {
        setDataByType([data["all_tag"], 100-parseFloat(data["all_tag"])])
    }, [])

    useEffect(()=>{
        setDataByType([data[valueType], 100-parseFloat(data[valueType])])
    }, [valueType])
    ...
    return (
        <div className="bar-chart w-full min-w-[320px] px-1">
            ...
            <div className="col-span-9 flex flex-row items-baseline gap-2">
                <div className="chart-title text-lg font-bold text-left col-span-4">{props.title}</div>
                <div className="chart-time text-md text-[#c2c1c1] col-span-4">
                ...
                onChange={(e) => setValueType(e.target.value)}>
                    <option value="all_tag"> 전체 ↓</option>
...
export default BarChartRateVer;